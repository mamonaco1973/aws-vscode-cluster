...
**Signing out.** `code-server` occupies the entire viewport and knows nothing about the broker's session, so the broker injects a small sign-out button into the workbench document. It mounts into the title bar, just left of the layout controls, and is styled from the workbench's own theme variables so it tracks light/dark and custom themes. If no title bar is found — a code-server release that renames the part, or a custom title bar turned off — it falls back to floating at the top right. It opens a confirmation dialog styled to match the sign-in page, since signing out stops the user's `code-server` process and discards unsaved editor state; files saved to the home directory are unaffected.
